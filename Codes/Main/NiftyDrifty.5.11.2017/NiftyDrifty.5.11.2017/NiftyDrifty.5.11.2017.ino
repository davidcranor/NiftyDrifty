...
void loop()
{
  ...
  if (sensorTicks >= SENSOR_TICKS_SAMPLE_SIZE)
  {
    ...
    currentRPM = (float)sensorTicks * 60000000 / (((float)currentTime - (float)lastRPMUpdateTime) * 20);
...
void updateLEDs()
{
  uint32_t rawGraphValue;
  uint32_t mappedGraphValue;

  if (currentRPM >= DANGER_RPM)
  {
    rawGraphValue = DANGER_RPM;
  } else {
    rawGraphValue = currentRPM;
  }

  mappedGraphValue = map(rawGraphValue, 0, DANGER_RPM, 0, NUM_LEDS);
